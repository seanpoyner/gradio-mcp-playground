...
class CLITestSuite:
    ...
    def test_server_lifecycle(self):
        ...
        for template in templates:
            ...
            result = self.run_cli_command(
                ["server", "create", server_name, "--template", template, "--port", "7861"]
            )
    ...
    def test_server_deletion(self):
        ...
        print("\n=== Testing Server Deletion ===")

        # Create a test server for deletion
        test_server_name = "test-delete-server"
        result = self.run_cli_command(
            ["server", "create", test_server_name, "--template", "basic", "--port", "7862"]
        )

        self.assert_test(
            result["success"],
            "Create test server for deletion",
            f"Server creation failed: {result.get('stderr', '')}",
        )
        ...
        result = self.run_cli_command(["server", "list"])
        self.assert_test(
            result["success"] and test_server_name in result.get("stdout", ""),
            "Server exists before deletion",
            f"Server not found in list: {result.get('stdout', '')}",
        )

        # Test deletion without --files flag (registry only)
        result = self.run_cli_command(["server", "delete", test_server_name, "--force"])

        self.assert_test(
            result["success"],
            "Delete server from registry",
            f"Server deletion failed: {result.get('stderr', '')}",
        )

        # Verify server is removed from registry
        result = self.run_cli_command(["server", "list"])
        self.assert_test(
            result["success"] and test_server_name not in result.get("stdout", ""),
            "Server removed from registry",
            f"Server still in list: {result.get('stdout', '')}",
        )

        # Create another test server for file deletion
        test_server_name_2 = "test-delete-files-server"
        result = self.run_cli_command(
            ["server", "create", test_server_name_2, "--template", "basic", "--port", "7863"]
        )

        self.assert_test(
            result["success"],
            "Create second test server for file deletion",
            f"Server creation failed: {result.get('stderr', '')}",
        )

        # Verify server directory exists
        server_dir = Path(test_server_name_2)
        self.assert_test(
            server_dir.exists(),
            "Server directory exists before deletion",
            f"Server directory {server_dir} not found",
        )

        # Test deletion with --files flag
        result = self.run_cli_command(
            ["server", "delete", test_server_name_2, "--force", "--files"]
        )

        self.assert_test(
            result["success"],
            "Delete server with files",
            f"Server deletion with files failed: {result.get('stderr', '')}",
        )

        # Verify server directory is removed
        self.assert_test(
            not server_dir.exists(),
            "Server directory removed",
            f"Server directory {server_dir} still exists",
        )

        # Test deletion of non-existent server
        result = self.run_cli_command(
            ["server", "delete", "nonexistent-server", "--force"], expected_return_code=1
        )

        self.assert_test(
            not result["success"],
            "Delete non-existent server (should fail)",
            "Should fail for nonexistent server",
        )
    ...
    def test_client_functionality(self):
        ...
        result = self.run_cli_command(
            ["client", "connect", "http://localhost:9999", "--name", "test-connection"],
    ...
    def test_registry_functionality(self):
        ...
        result = self.run_cli_command(["registry", "search", "--category", "tools"])
        self.assert_test(
            result["success"],
    ...
    def test_dev_commands(self):
        ...
        self.assert_test(
            result["success"], "Dev command help", f"Dev help failed: {result.get('stderr', '')}"
        )
    ...
    def test_error_handling(self):
        ...
        result = self.run_cli_command(
            ["server", "create", "test", "--template", "nonexistent"], expected_return_code=1
        )
        self.assert_test(
            not result["success"],
        ...
        result = self.run_cli_command(["server", "create"], expected_return_code=2)
        self.assert_test(
            not result["success"],
    ...
    def test_template_functionality(self):
        ...
        for template in templates_to_test:
            ...
            result = self.run_cli_command(
                ["server", "create", server_name, "--template", template["name"]]
            )
    ...
    def test_configuration_management(self):
        ...
        try:
            ...
            self.assert_test(False, "Configuration management", f"Configuration error: {str(e)}")
