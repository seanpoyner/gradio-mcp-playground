...
class MCPServerConfig:
    ...
    def _load_config(self):
        ...
        if self.config_file.exists():
            ...
            print(
                "Note: Servers requiring API keys (like brave-search) must be installed separately."
            )
    ...
    def export_claude_desktop_format(self) -> str:
        ...
        for name, server in self.config["mcpServers"].items():
            claude_server = {"command": server["command"]}
    ...
    def create_server_from_registry(
        ...
        self.add_server(name=server_id, command=command, args=args, env=env)

        return {"name": server_id, "command": command, "args": args, "env": env or {}}
