...
def create_unified_dashboard():
    ...
    with gr.Blocks(
                    ...
                    with gr.Tab("⚡ Quick Connect"):
                        ...
                        with gr.Row():
                            with gr.Column(scale=1):
                                ...
                                gr.Markdown("Secure file operations with configurable access controls.")
        ...
        def disconnect_mcp(connection_name):
            ...
            try:
                result = connection_manager.disconnect(connection_name)
                if result.get("success"):
                    return f"✅ Disconnected from {connection_name}"
                else:
                    return f"❌ Failed to disconnect: {result.get('error', 'Unknown error')}"
            except Exception as e:
            ...
            try:
                ...
                for conn in connections:
                    data.append([
                        ...
                        conn.get("url", ""),
                        conn.get("protocol", ""),
                        "Connected" if conn.get("connected", False) else "Disconnected",
                        conn.get("connected_since", "")
                    ])
                    choices.append(conn.get("name", ""))
        ...
        if HAS_CONFIG_MANAGER and HAS_REGISTRY:
            ...
            if 'filesystem_btn' in locals():
                filesystem_btn.click(
                    lambda: quick_connect_mcp("filesystem"),
                    outputs=[quick_connect_status]
                ).then(
            ...
            if 'memory_btn' in locals():
                memory_btn.click(
                    lambda: quick_connect_mcp("memory"),
                    outputs=[quick_connect_status]
                ).then(
                    lambda: gr.update(visible=True),
                    outputs=[quick_connect_status]
            ...
            if 'github_btn' in locals():
                github_btn.click(
                    lambda: quick_connect_mcp("github"),
                    outputs=[quick_connect_status]
                ).then(
                    lambda: gr.update(visible=True),
                    outputs=[quick_connect_status]
            ...
            if 'brave_btn' in locals():
                brave_btn.click(
                    lambda: quick_connect_mcp("brave-search"),
                    outputs=[quick_connect_status]
                ).then(
                    lambda: gr.update(visible=True),
                    outputs=[quick_connect_status]
            ...
            if 'refresh_btn' in locals():
                refresh_btn.click(
                    refresh_servers,
                    outputs=[servers_list, server_dropdown]
                )
            ...
            if 'install_filesystem_btn' in locals():
                install_filesystem_btn.click(
                    lambda: quick_install_popular("filesystem"),
            ...
            if 'install_memory_btn' in locals():
                install_memory_btn.click(
                    lambda: quick_install_popular("memory"),
                    outputs=[registry_install_status]
                ).then(
                    lambda: gr.update(visible=True),
                    outputs=[registry_install_status]
            ...
            if 'install_github_btn' in locals():
                install_github_btn.click(
                    lambda: quick_install_popular("github"),
                    outputs=[registry_install_status]
                ).then(
                    lambda: gr.update(visible=True),
                    outputs=[registry_install_status]
            ...
            if 'custom_mcp_connect_btn' in locals():
                custom_mcp_connect_btn.click(
                    lambda url, protocol: quick_connect_mcp(url) if HAS_CLIENT_MANAGER else "Client manager not available",
            ...
            if 'registry_install_btn' in locals():
                registry_install_btn.click(
                    install_registry_server,
                    inputs=[registry_server_selector, gr.Textbox(value="{}", visible=False)],  # Empty user args
                    outputs=[registry_install_status]
                )
            ...
            if 'save_settings_btn' in locals() and HAS_CONFIG_MANAGER:
                ...
                save_settings_btn.click(
                    save_settings,
                    inputs=[settings_port, settings_auto_reload, settings_protocol, settings_log_level],
            ...
            if 'connect_test_btn' in locals():
                def test_connection(url, protocol):
                    ...
                    if not HAS_CLIENT_MANAGER:
                        return gr.update(value={"error": "Client manager not available"}, visible=True)
                    
                    try:
                        # This would connect and get tools
                        # For now, return placeholder
                        return gr.update(value={"tools": ["example_tool_1", "example_tool_2"]}, visible=True)
                    except Exception as e:
        ...
        # Initialize on load - TEMPORARILY DISABLED TO DEBUG
        # if coding_agent:
        #     # Create combined load function
        #     def initialize_on_load():
        #         """Initialize UI elements on dashboard load"""
        #         # Initialize greeting
        #         greeting = initialize_general_greeting()
        #         
        #         # Load saved token
        #         if not config_manager.has_secure_storage():
        #             token_value = ""
        #             status_value = "Secure storage not available"
        #         else:
        #             saved_token = config_manager.load_secure_token("huggingface")
        #             if saved_token:
        #                 token_value = "*" * 20
        #                 status_value = "✅ Token loaded from secure storage (Using saved token)"
        #             else:
        #                 token_value = ""
        #                 status_value = "Enter your HuggingFace API token..."
        #         
        #         return greeting, token_value, status_value
        #     
        #     # Single load call for all initialization
        #     dashboard.load(initialize_on_load, outputs=[general_chatbot, hf_token_input, config_status])
