...
class ProtocolAdapter:
    ...
    def __init__(self, read_stream, write_stream, client_version="2025-03-26", server_version="2024-11-05"):
        ...
        if isinstance(message, dict):
            ...
            if "result" in message and isinstance(message["result"], dict):
                if "protocolVersion" in message["result"]:
                    logger.debug(f"Adapting protocol version from {message['result']['protocolVersion']} to {self.client_version}")
        ...
        if isinstance(message, dict):
            ...
            if message.get("method") == "initialize" and "params" in message:
                if "protocolVersion" in message["params"]:
                    logger.debug(f"Adapting protocol version from {message['params']['protocolVersion']} to {self.server_version}")
